...
from hiplib.crypto.dh import DH15
...
class HIPLib():
    ...
    def process_hip_packet(self, packet, ecbd_points):
        try:
            ...
            if hip_packet.get_packet_type() == HIP.HIP_I1_PACKET:
                logging.info("Spoke got I1 packet: dropping!!!");
                return []
                ...
                logging.info("Spoke got I2 packet: dropping!!!");
                return []
    ...
    def process_l2_frame(self, frame, ihit, rhit, src_str):
        try:
            ...
            elif hip_state.is_unassociated() or hip_state.is_closing() or hip_state.is_closed():
